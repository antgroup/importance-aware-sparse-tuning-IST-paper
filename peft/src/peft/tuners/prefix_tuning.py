...
class PrefixEncoder(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def forward(self, prefix: torch.Tensor):
        ...
        if self.scaling_mask is not None:
            past_key_values = past_key_values * self.scaling_mask[None, :]
        if self.trainable_mask is not None:
            past_key_values = past_key_values * (self.trainable_mask[None, :]) + past_key_values.detach() * (1 - (self.trainable_mask[None, :]))
